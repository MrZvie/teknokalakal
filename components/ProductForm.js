  ...
  return (
    <form onSubmit={saveProduct}>
      ...
      <div className="mb-2 flex flex-wrap gap-2">
        ...
        {!!images?.length && images.map(link => (
          <div key={link} className="h-32 mb-7 w-32">
            <img src={link} alt="" className="h-32 w-32 shadow-md mb-1 object-cover rounded-lg" />
            <button className="bg-redz text-white text-sm w-32 px-3 py-1 rounded-md">Remove</button>
          </div>
        ))}
        </ReactSortable>
        {isUploading && (
      ...
      <div className="flex gap-2">
        ...
        <button type="button" className="btn-red" onClick={() => router.push("/products")}>
          Cancel
        </button>
